...
**727 icons, drawn on one 24×24 grid, in three weights.** Built for shadcn/ui,
...
| --- | --- | --- |
| `stroke` | 727 | The full set. 2px keylines on a 24 grid. |
| `duotone` | 589 | The stroke drawing over a flat plate at reduced opacity. |
| `fill` | 540 | Solid, with the detail knocked back out of the shape. |

The table is one corner treatment. Every drawing is also cut with sharp corners,
so 3,712 SVGs in total.
...
55 icons come in a `square-` form and 60 in a `circle-` form, which wrap the
