...
**624 icons, drawn on one 24×24 grid, in three weights.** Built for shadcn/ui,
...
| --- | --- | --- |
| `stroke` | 624 | The full set. 2px keylines on a 24 grid. |
| `duotone` | 517 | The stroke drawing over a flat plate at reduced opacity. |
| `fill` | 469 | Solid, with the detail knocked back out of the shape. |

The table is one corner treatment. Every drawing is also cut with sharp corners,
so 3,220 SVGs in total.
