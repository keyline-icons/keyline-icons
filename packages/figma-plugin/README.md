...
Search the set inside Figma and drop an icon on the canvas. 1,178 names, four
styles, two corner treatments, MIT.
...
code.js         main thread: insert, drop, size, place, select, settings
ui.html         search, styles, shelves, grid. All of the UI, no build step
...
This copy of `icons.json` carries the keywords and the shelves as well as the
geometry, because the plugin reads exactly one URL and cannot go back for
`lib/icon-keywords.json` or `lib/icon-taxonomy.ts`. The CLI and MCP bundles are
the same bytes; one generator writes all three.

**The panel's own chrome is drawn from the same file.** The search mark, the
bell on each style tab and the square on the corner toggle are drawings out of
the bundle, never copies in `ui.html`. Their names are listed in `CHROME` at the
top of the script, and `build-data.mjs --check` fails if any of them is missing
from a style or a treatment, because a rename in the set reaches installed
copies over the CDN while the list only changes through review. Each shelf's
glyph travels in the bundle beside the shelf, so a rename there moves with the
...
**A click lands centred in the selected frame**, or centred in the viewport
...
somewhere the user did not point at. A frame nested inside an instance still
reads as a frame and still refuses, so the append is tried rather than
predicted and a refusal lands the icon on the page at the same point.

**A drag lands where it is let go.** A tile dragged out of the panel is posted
to Figma as `pluginDrop`, which comes back to `code.js` as a `drop` event with
the node under the pointer and the canvas point. The same parent rule applies:
dropped on a frame it goes inside, centred on the pointer; dropped on an
auto-layout frame it takes the slot between the two children it was let go
between; dropped on anything else it goes on the page. An icon is never a
parent, whether this run inserted it or not: a small square unfilled frame of
vector ink reads as one. Drops the panel did not start carry no Keyline
metadata and are left to Figma.

**An insert arrives at the size the footer is set to**, 16, 20, 24 or 32. The
frame is imported at 24 and then `rescale`d, which is Figma's scale tool, so the
stroke scales with it: a 16 draws a 1.33 line, exactly what the React component
draws at `size={16}`. Resizing the frame instead would keep a 2px line on a 16px
icon, which is a heavier drawing rather than a smaller one.
...
**Browsing is by shelf, searching is by rank.** With nothing typed, the grid is
the site's shelves in the rail's order, each under a heading that sticks while
its shelf is on screen and opens a list of every shelf to jump to. The shelves
come from the bundle, so a shelf the site opens reaches the panel with the data.
A query drops the shelves and shows one ranked list, because the best match
should lead wherever it is filed. A bundle from before the shelves were baked
has none, and the grid is one alphabetical run as it used to be.

**The panel remembers where it was left.** Style, corners, insert size and the
last eighteen names inserted live under one `figma.clientStorage` key on the
user's machine. The panel asks for them once it is listening and holds its
first render up to 400ms for them, so it does not draw stroke and then redraw
in duotone. An answer that arrives after the user has already chosen something
is dropped rather than applied over it. Recent is rebuilt with the grid rather than on every insert, because a
row that grows under the pointer moves every shelf below it.

**The keyboard works.** Typing anywhere searches, Enter in the field inserts the
top hit, the arrows move through the grid by position rather than by index, and
Escape goes back to the field.
