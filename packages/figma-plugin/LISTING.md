...
```
629 icons in three styles, cut rounded or sharp.
```
...
```
Search 629 icons and drop one on the canvas. No library to publish, no file to duplicate, no plan requirement.

Three styles

* Stroke: 629 icons, 2px, round caps and joins
...
Search knows more than the file names. 510 icons carry curated words, so "south" finds arrow-down, "hamburger" finds menu and "trash" finds bin. Paste a component name straight out of your code and it resolves: CheckCircle2 finds circle-check.
...
```
629 icons on a 24×24 grid, in three styles and two corner treatments: stroke, duotone and fill, cut rounded or sharp. MIT licensed, free for commercial work, no attribution required.

520 component sets, each with three variant properties, Container, Style and Corners, so you switch between regular, square and circle, between the three styles, and between rounded and sharp, without swapping components. 55 icons carry a square- form and 54 a circle-.
