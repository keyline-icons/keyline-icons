...
const radii = [0, 0.5, 1, 1.5];
const files = readdirSync('sharp').filter((f) => f.endsWith('.svg'));
const summary = radii.map((r) => ({ r, exact: 0, within01: 0, over: 0, worst: 0, worstName: '' }));

for (const file of files) {
  const target = paintedBBox(readPaths(readFileSync(`${K}/${file}`, 'utf8')), ROUND);
  const src = readFileSync(`sharp/${file}`, 'utf8');
  for (const [idx, r] of radii.entries()) {
    let out = src, ok = true;
    if (r > 0) {
      out = src.replace(/ d="([^"]+)"/g, (m, d) => { const v = refillet(d, r); if (v === null) { ok = false; return m; } return ` d="${v}"`; });
      if (r === 1) { mkdirSync('radius1', { recursive: true }); writeFileSync(`radius1/${file}`, out); }
      if (r === 0.5) { mkdirSync('radius05', { recursive: true }); writeFileSync(`radius05/${file}`, out); }
    }
    if (!ok) continue;
    const b = paintedBBox(readPaths(out), SHARP);
    const over = Math.max(target[0]-b[0], b[2]-target[2], target[1]-b[1], b[3]-target[3], 0);
    const s = summary[idx];
    if (over <= 0.001) s.exact++;
    else if (over <= 0.1) s.within01++;
    else s.over++;
    if (over > s.worst) { s.worst = over; s.worstName = file.replace('.svg',''); }
  }
}
for (const s of summary) console.log(`r=${s.r}\tinside the rounded box: ${s.exact}\twithin 0.1: ${s.within01}\tover: ${s.over}\tworst ${s.worst.toFixed(2)} (${s.worstName})`);
