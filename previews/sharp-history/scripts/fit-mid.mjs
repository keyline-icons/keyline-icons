// The middle rung: same de-filleted geometry, round join. With a round join the
// painted box is just the geometry box grown by a unit, so fitting geometry
// boxes is the whole job.
...
function xform(d,sx,sy,tx,ty){const X=v=>n(v*sx+tx),Y=v=>n(v*sy+ty);
  ...
  return out;}
let fitted = 0, worst = 0;
for (const f of readdirSync('mid').filter(x=>x.endsWith('.svg'))) {
  const rSrc = readFileSync(`${K}/${f}`,'utf8');
  let vSrc = readFileSync(`mid/${f}`,'utf8').replace('stroke-linejoin="miter"','stroke-linejoin="round"');
  const rP=[...rSrc.matchAll(/ d="([^"]+)"/g)].map(m=>m[1]), vP=[...vSrc.matchAll(/ d="([^"]+)"/g)].map(m=>m[1]);
  if (rP.length !== vP.length) { writeFileSync(`mid/${f}`, vSrc); continue; }
  let moved = 0;
  ...
  if (moved) fitted++;
  const T=pathBBox([...rSrc.matchAll(/ d="([^"]+)"/g)].map(m=>m[1]).join(' '));
  const B=pathBBox([...out.matchAll(/ d="([^"]+)"/g)].map(m=>m[1]).join(' '));
  if (T&&B) worst = Math.max(worst, Math.max(...B.map((v,q)=>Math.abs(v-T[q]))));
}
console.log({ middleRungFitted: fitted, worstBoxError: +worst.toFixed(3) });
